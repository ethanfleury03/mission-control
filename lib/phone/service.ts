...
const PHONE_SCHEMA_SQL: string[] = [
  ...
  `CREATE TABLE IF NOT EXISTS "phone_list_entries" (
    ...
    "createdAt" DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "updatedAt" DATETIME NOT NULL,
    CONSTRAINT "phone_list_entries_listId_fkey" FOREIGN KEY ("listId") REFERENCES "phone_lists" ("id") ON DELETE CASCADE ON UPDATE CASCADE
  )`,
  `CREATE TABLE IF NOT EXISTS "phone_campaigns" (
    ...
    "createdAt" DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "updatedAt" DATETIME NOT NULL,
    CONSTRAINT "phone_campaigns_listId_fkey" FOREIGN KEY ("listId") REFERENCES "phone_lists" ("id") ON DELETE CASCADE ON UPDATE CASCADE
  )`,
  `CREATE TABLE IF NOT EXISTS "phone_calls" (
    ...
    "createdAt" DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "updatedAt" DATETIME NOT NULL,
    CONSTRAINT "phone_calls_campaignId_fkey" FOREIGN KEY ("campaignId") REFERENCES "phone_campaigns" ("id") ON DELETE SET NULL ON UPDATE CASCADE,
    CONSTRAINT "phone_calls_listId_fkey" FOREIGN KEY ("listId") REFERENCES "phone_lists" ("id") ON DELETE SET NULL ON UPDATE CASCADE,
    CONSTRAINT "phone_calls_listEntryId_fkey" FOREIGN KEY ("listEntryId") REFERENCES "phone_list_entries" ("id") ON DELETE SET NULL ON UPDATE CASCADE
  )`,
  `CREATE TABLE IF NOT EXISTS "phone_call_events" (
    ...
    "createdAt" DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT "phone_call_events_phoneCallId_fkey" FOREIGN KEY ("phoneCallId") REFERENCES "phone_calls" ("id") ON DELETE CASCADE ON UPDATE CASCADE
  )`,
  ...
  `CREATE INDEX IF NOT EXISTS "phone_lists_status_idx" ON "phone_lists"("status")`,
  `CREATE INDEX IF NOT EXISTS "phone_lists_sourceType_idx" ON "phone_lists"("sourceType")`,
  `CREATE INDEX IF NOT EXISTS "phone_list_entries_listId_idx" ON "phone_list_entries"("listId")`,
  `CREATE INDEX IF NOT EXISTS "phone_list_entries_listId_phoneNormalized_idx" ON "phone_list_entries"("listId", "phoneNormalized")`,
  `CREATE INDEX IF NOT EXISTS "phone_list_entries_queueState_retryAfter_idx" ON "phone_list_entries"("queueState", "retryAfter")`,
  `CREATE INDEX IF NOT EXISTS "phone_list_entries_lastCallAt_idx" ON "phone_list_entries"("lastCallAt")`,
  `CREATE INDEX IF NOT EXISTS "phone_campaigns_status_idx" ON "phone_campaigns"("status")`,
  `CREATE INDEX IF NOT EXISTS "phone_campaigns_listId_idx" ON "phone_campaigns"("listId")`,
  `CREATE UNIQUE INDEX IF NOT EXISTS "phone_calls_providerCallId_key" ON "phone_calls"("providerCallId")`,
  `CREATE INDEX IF NOT EXISTS "phone_calls_campaignId_idx" ON "phone_calls"("campaignId")`,
  `CREATE INDEX IF NOT EXISTS "phone_calls_listId_idx" ON "phone_calls"("listId")`,
  `CREATE INDEX IF NOT EXISTS "phone_calls_listEntryId_idx" ON "phone_calls"("listEntryId")`,
  `CREATE INDEX IF NOT EXISTS "phone_calls_providerStatus_idx" ON "phone_calls"("providerStatus")`,
  `CREATE INDEX IF NOT EXISTS "phone_calls_disposition_idx" ON "phone_calls"("disposition")`,
  `CREATE INDEX IF NOT EXISTS "phone_calls_startedAt_idx" ON "phone_calls"("startedAt")`,
  `CREATE INDEX IF NOT EXISTS "phone_calls_createdAt_idx" ON "phone_calls"("createdAt")`,
  `CREATE INDEX IF NOT EXISTS "phone_call_events_phoneCallId_createdAt_idx" ON "phone_call_events"("phoneCallId", "createdAt")`,
];
...
async function ensurePhoneSchema(): Promise<void> {
  ...
  phoneSchemaReady = (async () => {
    const existing = await prisma.$queryRawUnsafe<Array<{ name: string }>>(
      `SELECT name FROM sqlite_master WHERE type='table' AND name IN (${PHONE_TABLE_NAMES.map((name) => `'${name}'`).join(', ')})`,
    );

    const existingNames = new Set(existing.map((row) => row.name));
    if (PHONE_TABLE_NAMES.every((name) => existingNames.has(name))) return;

    for (const statement of PHONE_SCHEMA_SQL) {
