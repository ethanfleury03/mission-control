...
function buildHeatLegend(title: string, buckets: Array<{ count: number }>, totalContacts: number): GeoHeatLegend {
  const maxCount = Math.max(...buckets.map((bucket) => bucket.count), 0);
  const steps = [
    { min: 1, max: 10, color: '#fca5a5' },
    { min: 11, max: 99, color: '#fb7185' },
    { min: 100, max: 999, color: '#ef4444' },
    { min: 1000, max: 4999, color: '#dc2626' },
    { min: 5000, max: null, color: '#991b1b' },
  ];
  ...
  return {
    ...
    maxCount,
    bands: steps
      .filter((step) => maxCount > 0 && step.min <= maxCount)
      .map((step) => ({
        ...step,
        label: step.max === null ? `${step.min.toLocaleString()}+` : `${step.min.toLocaleString()} - ${step.max.toLocaleString()}`,
      })),
  };
}
...
export async function buildGeoDashboardSnapshot(filters: GeoDashboardRequest): Promise<GeoDashboardSnapshot> {
  ...
  return {
    ...
    topCities,
    countryBuckets,
    heatLegend: buildHeatLegend('Contacts', countryBuckets, mappedRows.length),
    topCountries,
    topStates,
    topOwners,
...
export async function buildGeoCountryDrilldownSnapshot(
  ...
  return {
    ...
    stateBuckets,
    heatLegend: buildHeatLegend('Contacts', stateBuckets, rows.length),
