...
- **Billing** enabled on the project (required for Cloud Storage, Cloud Build uploads, etc.). Bootstrap uses the **global** Cloud Build API (`gcloud builds submit` **without** `--region`). Calling `POST .../locations/us-central1/builds` returns **404 NOT_FOUND** on many new projects until Google enables regional workers — that is unrelated to your tarball. Staging uses **`gs://PROJECT_ID_cloudbuild/source`** with **`--gcs-source-staging-dir`**. Build configs do not set `logging: CLOUD_LOGGING_ONLY` (avoids another class of first-run failures).
