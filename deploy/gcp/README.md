...
- **Billing** enabled on the project (required for Cloud Storage, Cloud Build uploads, etc.). Bootstrap uses the **global** Cloud Build API (`gcloud builds submit` **without** `--region`). The bucket **`gs://PROJECT_ID_cloudbuild` must be in multi-region `US`** (not a single region like `us-central1`); a regional bucket causes **NOT_FOUND** after the source tarball uploads. If you hit that, delete the bucket (empty it first), then re-run bootstrap. Build configs set **`logsBucket: ${_LOGS_BUCKET}`** (bootstrap passes the full `gs://.../logs` URI; **`$PROJECT_ID_suffix` is invalid** in Cloud Build substitution syntax).
