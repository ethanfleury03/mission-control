...
- **Billing** enabled on the project (required for Cloud Storage, Cloud Build uploads, etc.). Bootstrap uses the **global** Cloud Build API (`gcloud builds submit` **without** `--region`). The bucket **`gs://PROJECT_ID_cloudbuild` must be in multi-region `US`**. Build **`logsBucket`** must be **`gs://BUCKET`** only (not `.../logs`); a wrong URI causes **NOT_FOUND** after upload. Bootstrap passes **`_LOGS_BUCKET=gs://PROJECT_cloudbuild`** (bucket root).
