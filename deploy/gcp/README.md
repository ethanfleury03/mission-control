...
- **Billing** enabled on the project (required for Cloud Storage, Cloud Build uploads, etc.). With **`builds/region`** set to a region (not `global`), Cloud Build stages source in **`gs://PROJECT_ID_<region>_cloudbuild/source`**. Bootstrap creates that bucket and passes **`--gcs-source-staging-dir`**. The build configs **do not** set `logging: CLOUD_LOGGING_ONLY`; forcing Cloud Logging only on a brand-new project can make **`builds.create` return NOT_FOUND** until the regional default log bucket exists. Default (legacy) logging uses Cloud Storage and is more reliable for first deploys.
