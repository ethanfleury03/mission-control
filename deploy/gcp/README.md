...
- **Billing** enabled on the project (required for Cloud Storage, Cloud Build uploads, etc.). If `gcloud builds submit` returns **NOT_FOUND** when uploading to `gs://PROJECT_ID_cloudbuild`, the bootstrap script now creates that bucket, enables `storage.googleapis.com`, sets **`gcloud config set builds/region <REGION>`**, and passes **`--region=<REGION>`** on every submit (using `global` breaks regional Cloud Build). If it still fails, confirm billing and that your account can create objects in that bucket (bootstrap grants `storage.objectAdmin` on the bucket to you and to the Cloud Build service account).
