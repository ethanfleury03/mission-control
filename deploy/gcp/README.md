...
- **Billing** enabled on the project (required for Cloud Storage, Cloud Build uploads, etc.). Bootstrap uses the **global** Cloud Build API (`gcloud builds submit` **without** `--region`). The bucket **`gs://PROJECT_ID_cloudbuild` must be in multi-region `US`**. After enabling APIs, bootstrap **waits up to ~3 minutes** for **`PROJECT_NUMBER@cloudbuild.gserviceaccount.com`** to appear; if it never does (rare org policy), it grants the staging bucket to **`PROJECT_NUMBER-compute@developer.gserviceaccount.com`** instead and enables **Compute Engine API** (creates that SA). Build configs use **default logging**.
