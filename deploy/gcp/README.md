...
- **Billing** enabled on the project (required for Cloud Storage, Cloud Build uploads, etc.). Bootstrap uses the **global** Cloud Build API (`gcloud builds submit` **without** `--region`). The bucket **`gs://PROJECT_ID_cloudbuild` must be in multi-region `US`**. If **`builds.create` returns NOT_FOUND** after the tarball uploads, the default **`PROJECT_NUMBER@cloudbuild.gserviceaccount.com`** may be missing: disable then re-enable the **Cloud Build API** in the console (bootstrap checks and prints exact steps). Build configs use **default logging** (no custom `logsBucket` / `GCS_ONLY`) to avoid another class of first-run failures.
