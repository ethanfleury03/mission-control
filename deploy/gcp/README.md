...
- **Billing** enabled on the project (required for Cloud Storage, Cloud Build uploads, etc.). With **`builds/region`** set to a region (not `global`), Cloud Build stages source in **`gs://PROJECT_ID_<region>_cloudbuild/source`** (e.g. `..._us-central1_cloudbuild`), **not** `gs://PROJECT_ID_cloudbuild`. Bootstrap creates the **regional** bucket and passes **`--gcs-source-staging-dir`** on every `gcloud builds submit` so the upload path and API agree (mismatch causes **NOT_FOUND** after upload). It also provisions the Cloud Build service agent and grants **`roles/storage.admin`** on that bucket to the Cloud Build SA, service agent, and submitter.
