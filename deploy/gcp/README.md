...
- **Billing** enabled on the project (required for Cloud Storage, Cloud Build uploads, etc.). If `gcloud builds submit` returns **NOT_FOUND** after uploading the tarball, bootstrap now (a) runs `gcloud beta services identity create --service=cloudbuild.googleapis.com`, (b) grants **`roles/cloudbuild.serviceAgent`** to `service-PROJECT_NUMBER@gcp-sa-cloudbuild.iam.gserviceaccount.com`, and (c) grants **`roles/storage.admin`** on the staging bucket to the legacy Cloud Build SA, that service agent, and your submitter account — Cloud Build needs **`storage.buckets.get`** on the bucket, not only object ACLs. Also sets **`builds/region`** and **`--region`** on every submit.
