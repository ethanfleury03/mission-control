...
mc_info "Applying RAG support schema to $MC_APP_DB"
bash "$SCRIPT_DIR/apply-rag-schema.sh" "$PROJECT_ID" "$REGION" "$MC_SQL_INSTANCE" "$MC_APP_DB" "$MC_SQL_USER" "$SQL_PASS"
...
OUTREACH_DEEP_SYNC_ENABLED="false"
if [ "$MC_ENV" = "prod" ]; then
  OUTREACH_DEEP_SYNC_ENABLED="true"
fi
gcloud builds submit "$REPO_ROOT" \
  --gcs-source-staging-dir="$GCS_SOURCE_STAGING" \
  --config="$SCRIPT_DIR/cloudbuild.web.yaml" \
  --substitutions="_REGION=$REGION,_AR_REPO=$MC_AR_REPO,_SERVICE=$MC_WEB_SERVICE,_SERVICE_ACCOUNT=$MC_WEB_SA,_API_URL=$API_URL,_CLOUD_SQL_INSTANCE=$CLOUD_SQL_CONN,_APP_DB_SECRET=$MC_APP_DB_SECRET,_AUTH_SECRET=$MC_AUTH_SECRET,_GOOGLE_ID_SECRET=$MC_GOOGLE_ID_SECRET,_GOOGLE_SECRET_SECRET=$MC_GOOGLE_SECRET_SECRET,_OPENROUTER_SECRET=$MC_OPENROUTER_SECRET,_OPENAI_SECRET=$MC_OPENAI_SECRET,_IMAGE_OPENROUTER_SECRET=$MC_IMAGE_OPENROUTER_SECRET,_FIRECRAWL_SECRET=$MC_FIRECRAWL_SECRET,_SERPER_SECRET=$MC_SERPER_SECRET,_HUBSPOT_TOKEN_SECRET=$MC_HUBSPOT_TOKEN_SECRET,_HUBSPOT_PORTAL_SECRET=$MC_HUBSPOT_PORTAL_SECRET,_RAPIDAPI_SECRET=$MC_RAPIDAPI_SECRET,_GOOGLE_SA_EMAIL_SECRET=$MC_GOOGLE_SA_EMAIL_SECRET,_GOOGLE_SA_KEY_SECRET=$MC_GOOGLE_SA_KEY_SECRET,_OUTREACH_SERVICE_TOKEN_SECRET=$MC_OUTREACH_SERVICE_TOKEN_SECRET,_OUTREACH_WEBHOOK_SECRET_SECRET=$MC_OUTREACH_WEBHOOK_SECRET_SECRET,_OUTREACH_OPENCLAW_GATEWAY_URL_SECRET=$MC_OUTREACH_OPENCLAW_GATEWAY_URL_SECRET,_OUTREACH_OPENCLAW_WEBHOOK_URL_SECRET=$MC_OUTREACH_OPENCLAW_WEBHOOK_URL_SECRET,_RETELL_API_SECRET=$MC_RETELL_API_SECRET,_PHONE_RETELL_AGENT_IDS_SECRET=$MC_PHONE_RETELL_AGENT_IDS_SECRET,_PHONE_RETELL_AGENT_ID_SECRET=$MC_PHONE_RETELL_AGENT_ID_SECRET,_PHONE_RETELL_PROFILE_KEY_SECRET=$MC_PHONE_RETELL_PROFILE_KEY_SECRET,_PHONE_RETELL_PROFILE_LABEL_SECRET=$MC_PHONE_RETELL_PROFILE_LABEL_SECRET,_PHONE_RETELL_CONVERSATION_FLOW_ID_SECRET=$MC_PHONE_RETELL_CONVERSATION_FLOW_ID_SECRET,_PHONE_RETELL_OUTBOUND_NUMBER_SECRET=$MC_PHONE_RETELL_OUTBOUND_NUMBER_SECRET,_PHONE_RETELL_OUTBOUND_NUMBER_LABEL_SECRET=$MC_PHONE_RETELL_OUTBOUND_NUMBER_LABEL_SECRET,_PHONE_RETELL_VOICE_LABEL_SECRET=$MC_PHONE_RETELL_VOICE_LABEL_SECRET,_PHONE_RETELL_WEBHOOK_STATUS_SECRET=$MC_PHONE_RETELL_WEBHOOK_STATUS_SECRET,_OUTREACH_DEEP_SYNC_ENABLED=$OUTREACH_DEEP_SYNC_ENABLED,_HELP_DESK_AI_PLAN_MODEL=${HELP_DESK_AI_PLAN_MODEL:-anthropic/claude-opus-4.7},_HELP_DESK_AI_PLAN_MAX_TOKENS=${HELP_DESK_AI_PLAN_MAX_TOKENS:-8000}"
...
gcloud run services update "$MC_WEB_SERVICE" --region="$REGION" --project="$PROJECT_ID" \
  --update-env-vars "NEXTAUTH_URL=$AUTH_PUBLIC_URL,AUTH_URL=$AUTH_PUBLIC_URL,OUTREACH_CRM_CALLBACK_URL=$AUTH_PUBLIC_URL/api/outreach-crm/v1/openclaw/callback,IMAGE_OPENROUTER_CHAT_MODEL=$MC_IMAGE_CHAT_MODEL,IMAGE_OPENROUTER_IMAGE_MODEL=$MC_IMAGE_IMAGE_MODEL,IMAGE_OPENROUTER_VIDEO_MODEL=$MC_IMAGE_VIDEO_MODEL" >/dev/null
...
add_web_secret OPENAI_API_KEY "$MC_OPENAI_SECRET"
...
add_web_secret RAPIDAPI_KEY "$MC_RAPIDAPI_SECRET"
...
add_web_secret OUTREACH_CRM_SERVICE_TOKEN "$MC_OUTREACH_SERVICE_TOKEN_SECRET"
add_web_secret OUTREACH_CRM_WEBHOOK_SECRET "$MC_OUTREACH_WEBHOOK_SECRET_SECRET"
add_web_secret OUTREACH_CRM_OPENCLAW_GATEWAY_URL "$MC_OUTREACH_OPENCLAW_GATEWAY_URL_SECRET"
add_web_secret OUTREACH_CRM_OPENCLAW_WEBHOOK_URL "$MC_OUTREACH_OPENCLAW_WEBHOOK_URL_SECRET"
add_web_secret RETELL_API_KEY "$MC_RETELL_API_SECRET"
add_web_secret PHONE_RETELL_AGENT_IDS "$MC_PHONE_RETELL_AGENT_IDS_SECRET"
add_web_secret PHONE_RETELL_AGENT_ID "$MC_PHONE_RETELL_AGENT_ID_SECRET"
add_web_secret PHONE_RETELL_PROFILE_KEY "$MC_PHONE_RETELL_PROFILE_KEY_SECRET"
add_web_secret PHONE_RETELL_PROFILE_LABEL "$MC_PHONE_RETELL_PROFILE_LABEL_SECRET"
add_web_secret PHONE_RETELL_CONVERSATION_FLOW_ID "$MC_PHONE_RETELL_CONVERSATION_FLOW_ID_SECRET"
add_web_secret PHONE_RETELL_OUTBOUND_NUMBER "$MC_PHONE_RETELL_OUTBOUND_NUMBER_SECRET"
add_web_secret PHONE_RETELL_OUTBOUND_NUMBER_LABEL "$MC_PHONE_RETELL_OUTBOUND_NUMBER_LABEL_SECRET"
add_web_secret PHONE_RETELL_VOICE_LABEL "$MC_PHONE_RETELL_VOICE_LABEL_SECRET"
add_web_secret PHONE_RETELL_WEBHOOK_STATUS "$MC_PHONE_RETELL_WEBHOOK_STATUS_SECRET"
